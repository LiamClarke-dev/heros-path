-- Hero's Path Production Migration
-- Run this against the production database to bring it up to date with the current schema.
...
CREATE UNIQUE INDEX IF NOT EXISTS users_replit_id_unique ON users (replit_id)
  WHERE replit_id IS NOT NULL;

-- Copy avatar_url into the new column for any existing rows
UPDATE users SET profile_image_url = avatar_url WHERE profile_image_url IS NULL AND avatar_url IS NOT NULL;

-- ============================================================
-- 2. SESSIONS TABLE (completely new schema for express-session)
-- ============================================================
...
-- ============================================================
-- 3. PLACE_CACHE TABLE — add missing columns
-- ============================================================
...
-- Migrate cached_at -> updated_at where not yet set
UPDATE place_cache SET updated_at = cached_at WHERE updated_at IS NULL AND cached_at IS NOT NULL;

-- ============================================================
-- 4. JOURNEYS TABLE — add new columns
-- ============================================================
...
CREATE UNIQUE INDEX IF NOT EXISTS exploration_sharing_pair_unique ON exploration_sharing (user_id, friend_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS list_shares_list_user_unique ON list_shares (list_id, shared_with_user_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS list_collaborators_list_user_unique ON list_collaborators (list_id, user_id);
